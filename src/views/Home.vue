<template>
  <div class="flex justify-center items-center h-screen">
    ...
    <div class="relative mx-auto" style="max-width: 300px; width: calc(1ch * 28);">
      ...
      <!-- Liste déroulante -->
      <ul
        v-if="afficherListe && departementsFiltres.length"
        class="border bg-white max-h-60 overflow-y-auto shadow-lg absolute left-0 top-full w-full"
      >
        <li
...
export default {
  data() {
    return {
      ...
      afficherListe: false, // Contrôle l'affichage de la liste
      departements: [
        { code: "01", nom: "Ain" },
        { code: "02", nom: "Aisne" },
        { code: "03", nom: "Allier" },
      ...
      departementSelectionne: null, // Garde en mémoire le département sélectionné
    };
  },

  computed: {
    departementsFiltres() {
      // Filtre les départements par recherche
      return this.departements.filter(
        (dep) =>
          dep.nom.toLowerCase().includes(this.recherche.toLowerCase()) ||
          dep.code.toLowerCase().includes(this.recherche.toLowerCase())
      );
    },
  },
  methods: {
    naviguerVersDepartement(departement) {
      // Redirige directement à la page du département
      this.$router.push(`/departements/${departement.code}`);
      this.recherche = ""; // Réinitialise la recherche
      this.afficherListe = false; // Masque la liste
    },
    masquerListe() {
      // Garde un petit délai pour permettre le clic
      setTimeout(() => {
